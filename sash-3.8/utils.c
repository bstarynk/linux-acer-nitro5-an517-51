 ...
 */
BOOL
copyFile (const char *srcName, const char *destName, BOOL setModes)
{
  int rfd;
  int wfd;
  int rcc;
  ...
  *retArgv = NULL;
  ...
  while (*cp)
	  ...
	  if (ch == '\\')
	    {
	      ...
	      if (ch == '\0')
		{
		  fprintf (stderr, "Bad quoted character\n");
	...
	*cp++ = '\0';
	...
	{
	  fprintf (stderr, "Cannot use quoted and unquoted wildcards\n");
 ...
 */
BOOL
makeString (int argc, const char **argv, char *buf, int bufLen)
{
  int len;
