...
static const char *const version = "3.8-basile";
...
readFile (const char *name)
{
  ...
  while (TRUE)
      ...
      if (ttyFlag)
	{
	  ...
	  dynbuf = readline (basile_prompt ());
	  bufsiz = dynbuf ? (strlen (dynbuf + 1)) : 0;
	}
      else
	{
	  bufsiz = getline (&dynbuf, &buflen, fp);
	};
