...
static const char *convertName (const CONVERT * table, const char *inFile);
...
gzip (const char *inputFileName, const char *outputFileName)
{
  ...
  while ((len = read (inFD, buf, sizeof (buf))) > 0)
	...
	{
	  fprintf (stderr, "%s: %s\n", inputFileName, gzerror (outGZ, &err));
...
gunzip (const char *inputFileName, const char *outputFileName)
{
  ...
  if (len < 0)
    {
      fprintf (stderr, "%s: %s\n", inputFileName, gzerror (inGZ, &err));
  ...
  for (;;)
    {
	  ...
	  (memcmp (table->input,
		   inputFile + inputLength - testLength, testLength) == 0))
	{
	  break;
	}
